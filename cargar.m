...
senal = load("Grupo6_senal_tres.mat").senal;
...
% senal senoidal reconstruida
senal_senoidal = senal - senal_reconstruida;
[X_senoidal, frec_senoidal] = freqz(senal_senoidal, 1, 16384, fs);

%% HALLAR IMPULSOS

error = (senal - senal_senoidal).^2;
...
for i=1:length(indexp)
    disp("Impulso en muestra: " + indexp(i))
end
...
xlabel('Frecuencia'),
ylabel('Modulo'),
title('[SEÑAL INGRESADA] Espectro de modulo')
...
subplot(3,1,3)
plot(frec_senoidal, abs(X_senoidal)),
grid on,
xlabel('Frecuencia'),
ylabel('Modulo'),
title('[SEÑAL SENOIDAL] Espectro de modulo')
...
hold on
stem(t, senal, 'b', '.');
stem(t(indexp), senal(indexp), 'r');
grid on, xlabel('Tiempo'), ylabel('Señal'),
title('Impulsos detectados')
